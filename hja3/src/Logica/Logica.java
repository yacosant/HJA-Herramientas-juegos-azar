...
public class Logica {
    ...
    private void generarJugadores() { //debuguear
    ...
    private double[] calcularPorcentajes() {
        ...
        if (estado != -1) {
            for (int i = 0; i < 6; i++) {
                por[i] = redondear(100 * jugadores[i].getVictorias() / NUM_VUELTAS, 3);
            }
        } else {
            for (int j = 0; j < 6; j++) {
    ...
    private void remove(Carta c) {
        for (int i = 0; i < baraja.size(); i++) {
            if (baraja.get(i).getValor() == c.getValor() && baraja.get(i).getColor() == c.getColor()) {
                baraja.remove(i);
            }
        }
    }
